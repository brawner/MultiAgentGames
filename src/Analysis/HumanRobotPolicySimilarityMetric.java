...
public class HumanRobotPolicySimilarityMetric {
	...
	public static void main(String[] args){
		...
		String pathToExperiment = "/home/ng/workspace/Experiment1/Batch_fromUpDown/";
		...
		File humanExperimentLocation = new File(pathToExperimentHumanTrials);
		File agentExperimentLocation = new File(pathToExperimentAgentTrials);

		String[] humanGameFiles = humanExperimentLocation.list(new FilenameFilter() {
			@Override
			public boolean accept(File current, String name) {
				return name.toLowerCase().contains(".game");
			}
		});

		String[] agentGameFiles = agentExperimentLocation.list(new FilenameFilter() {
			@Override
			public boolean accept(File current, String name) {
				return name.toLowerCase().contains(".game");
			}
		...
		Map<String, HashMap<String, ArrayList<String>>> humanFileMap = getDataStructure(humanGameFiles);

		Map<String, HashMap<String, ArrayList<String>>> agentFileMap = getDataStructure(agentGameFiles);
		...
		for(String trialStr: humanFileMap.keySet()){
			...
			for(String matchStr : humanFileMap.get(trialStr).keySet()){

				if(!agentFileMap.containsKey(trialStr) || !agentFileMap.get(trialStr).containsKey(matchStr)){
					System.err.println("Trial " + trialStr + " or match " + matchStr + " does not exist.");
					System.exit(-100);
				}
...
//				System.out.println("trial " + agentFileMap.containsKey(trialStr)  + " match " +  agentFileMap.get(trialStr).containsKey(matchStr));
				...
				for(String humanFile :humanFileMap.get(trialStr).get(matchStr)){
					humanListOfFiles.add(new File(pathToExperimentHumanTrials + humanFile));
				}
				
				for(String agentFile :agentFileMap.get(trialStr).get(matchStr)){
					agentListOfFiles.add(new File(pathToExperimentAgentTrials + agentFile));
				}
...
//					System.out.println("human path as seen right now " + humanListOfFiles.get(i).getAbsolutePath() );
				...
				outputString = outputString + trialStr + "/" +matchStr +  ","  + actionSimilarity + "," + stateSimilarity +"," +((double)actionSimilarity)/stateSimilarity +"," +((double)stateSimilarity)/humanStateActionCount.size() +"\n";
	...
	public static Map<String,HashMap<String, ArrayList<String>>> getDataStructure(String[] gameFiles){
		Map<String, HashMap<String, ArrayList<String>>> fileMap = new HashMap<String, HashMap<String, ArrayList<String>>>();

		for(String fileName : gameFiles){
			String[] parts = fileName.split("_");
			// check if humanFileMap has a trial
			String trialStr =parts[0] + "_"+ parts[1];
			String matchStr =parts[2] + "_"+ parts[3];

			if(fileMap.containsKey(trialStr)){
				if(fileMap.get(trialStr).containsKey(matchStr)){
					fileMap.get(trialStr).get(matchStr).add(fileName);
				}
				else{
					fileMap.get(trialStr).put(matchStr, new ArrayList<String>());
					fileMap.get(trialStr).get(matchStr).add(fileName);
				}
			}
			else{
				fileMap.put(trialStr, new HashMap<String, ArrayList<String>>());
				fileMap.get(trialStr).put(matchStr, new ArrayList<String>());
				fileMap.get(trialStr).get(matchStr).add(fileName);
			}
